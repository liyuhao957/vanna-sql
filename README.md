...
- 规则管理区已从原有Tab中移出，作为独立的"业务规则管理"块，位于页面下方，横向铺满且居中，最大宽度1200px。
- 这样可提升规则查阅、维护的便捷性和可读性。
- 样式已在`frontend/style.css`中补充，无需额外配置。
...
本系统支持"结果不符时的SQL优化"功能，帮助你在SQL语句无报错但结果不符合预期时，智能优化SQL。

### 入口
- 在SQL修正Tab区，切换到"结果不符时的SQL优化"Tab。

### 用法
1. 原始SQL：自动填充为最近一次生成的SQL（如无则手动粘贴）。
2. 实际查询结果：可选，粘贴部分数据或留空。
3. 真实需求描述：必填，说明你想要的结果。
4. 点击"优化SQL"按钮，系统会自动拼接RAG上下文（表结构、字段说明、历史示例等），调用大模型智能优化SQL。
5. 优化结果会展示在下方，可一键复制。

### 修正原则
- 系统会自动检索与你需求最相关的数据库结构、字段说明、历史SQL等上下文，辅助大模型理解你的业务。
- 优化结果仅供参考，最终执行前请自行审核。

### 操作示意
1. 进入网页端，找到SQL修正Tab区，切换到"结果不符时的SQL优化"。
2. 填写相关内容，点击优化按钮。
3. 复制优化后的SQL到数据库执行。
